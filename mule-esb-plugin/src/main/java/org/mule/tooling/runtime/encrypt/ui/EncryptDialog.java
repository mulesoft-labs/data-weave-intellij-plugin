package org.mule.tooling.runtime.encrypt.ui;
...
public class EncryptDialog extends DialogWrapper {
    ...
    protected ValidationInfo doValidate() {
        ...
        if (encKey == null || encKey.length() < selectedAlgorithm.getMinKeySize()) {
            validationInfo = new ValidationInfo("The encryption key for the algorithm " + selectedAlgorithm.toString() + " must be at least " + selectedAlgorithm.getMinKeySize() + " characters long",
                    this.encryptionKey);
        }

        if (encKey != null && encKey.length() > selectedAlgorithm.getMaxKeySize()) {
            validationInfo = new ValidationInfo("The encryption key for the algorithm " + selectedAlgorithm.toString() + " must be no more than " + selectedAlgorithm.getMaxKeySize() + " characters long",
                    this.encryptionKey);
        }
